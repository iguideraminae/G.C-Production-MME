...
class QcInspection(models.Model):
    ...
    product_id = fields.Many2one(
        ...
        store=True,
        help="Product associated with the inspection", string="Produit"
    )
    ...
    def unlink(self):
        for inspection in self:
            ...
            if inspection.state != "draft":
                raise exceptions.UserError(
                    _("Vous ne pouvez pas supprimer un contrôle qui n’est pas à l’état de brouillon.")
                )
        return super().unlink()
        ...
        for inspection in self:
            for line in inspection.inspection_lines:
                if line.question_type == "qualitative" and not line.qualitative_value:
                    raise exceptions.UserError(
                        _(
                            "Vous devez fournir une réponse à toutes "
                            "les valeurs."
                        )
                    )
                    ...
                    raise exceptions.UserError(
                        _(
                            "Vous devez fournir une unité de mesure "
                            "les valeurs."
                        )
                    )
...
class QcInspectionLine(models.Model):
    ...
    question_type = fields.Selection(
        [("qualitative", "Valeur sélectionnée"), ("quantitative", "Valeur numérique")],
        readonly=True,
    )
    ...
    success = fields.Boolean(
        compute="_compute_quality_test_check", string="Succès?", store=True
    )
